...
| 迭代 | 共同目标 | 各成员可交付结果 |
| --- | --- | --- |
| S0 | 契约与环境 | M1 环境（认证推迟至 S1，见 #45）；M2 页面骨架；M3 答疑 schema/样例；M4 数字人组件接口；M5 表模型/API；M6 学习事件/教师原型 |
| S1 | 最小文字学习闭环 | M1 交付登录/鉴权并联调（#45/#16）；M2 文字聊天/课程页；M3 单门课 PDF 检索答疑；M4 2D 形象与播报；M5 作业/错题/计划；M6 课件上传/基础统计 |
