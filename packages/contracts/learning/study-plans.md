...
| `items[].chapterId` | string \| null | 否 | 关联章节 |
| `items[].knowledgePoint` | string \| null | 否 | 知识点标注 |
...
持久化、提供编辑与提醒。**不**承担建议生成职责。

## 资源：StudyPlan

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `id` | string | 是 | 形如 `plan_6d8a3c` |
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `id` | string | 是 | 形如 `plan_item_3a1b2c` |
  ...
  "items": [
    {
      ...
      "order": 1,
      "relatedMistakeId": "mistake_9c4b2e",
...
> 中，`content` 映射为 PlanItem 的 `content`，`suggestedDueAt` 映射为 PlanItem 的 `dueAt`，
> `chapterId` 映射为 PlanItem 关联章节（与计划级 `chapterId` 就高取非空值）。
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `id` | string | 是 | 形如 `reminder_5e8d2a` |
...
6. **站内提醒只由服务端生成。** 学生不能 `POST /api/v1/reminders` 自建提醒，
   只能调整计划/作业的 `dueAt`，由服务端扫描时生成对应提醒。
7. **提醒 `isRead = true` 后 `readAt` 必填。** 标记未读时 `readAt` 清空为 null。
8. **已删除计划项的关联提醒一并删除。** 不悬空引用。
...
| `TUTORING_TURN_NOT_FOUND` | 422 | `source = "tutoring"` 但 `sourceTurnId` 校验失败 |
