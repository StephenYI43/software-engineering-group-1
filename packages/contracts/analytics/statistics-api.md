...
| `page` | int | 否 | `1` | `students` 分页页码，从 1 起；超出总页数时 `items` 为空数组 |
| `pageSize` | int | 否 | `50` | 每页学生数，1—200 |

`last7d` / `last30d` 的定义：开区间端点 `to` 取**生成时刻在 `timezone` 下所处日的 00:00**
（即「今天」不计入），向前取 7 / 30 个完整自然日（区间左闭右开）。例如生成于 2026-09-17
（`Asia/Shanghai`）时，`to = 2026-09-17T00:00+08:00`（即 `2026-09-16T16:00:00Z`），
`last7d` 覆盖 09-10 至 09-16。服务端**不得**依赖服务器本地时区
